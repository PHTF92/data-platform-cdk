...
class RawDMSRole(iam.Role):
    """[DMS role for the raw data lake]

    inherits: aws_cdk.aws_iam

    Args:
        scope (core.Construct): The application scope
        deploy_env (Enviroment): The environment used
        data_lake_raw_bucket (BaseDataLakeBucket): The bucket to give access
    """
...
class OrdersDMS(dms.CfnReplicationTask):
    """[The migration service task]

    inherits: replicationTask

    Args:
        scope (core.Construct): The application scope
        common_stack (RDS_instance): The RDS instance that will have the data migrated
        data_lake_raw_bucket (BaseDataLakeBucket): The bucket to save the files
    """
...
class DmsStack(core.Stack):
    """[The class to create the stack for the DMS]
