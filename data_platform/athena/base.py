...
class BaseAthenaBucket(s3.Bucket):
    def __init__(
        self, scope: core.Construct, deploy_env: ae.active_environment, **kwargs
    ) -> None:
        self.deploy_env = deploy_env
        self.obj_name = f"s3-phtf-{self.deploy_env.value}-data-lake-athena-results"
...
class BaseAthenaWorkgroup(athena.CfnWorkGroup):
    def __init__(
        ...
        **kwargs,
        ...
        self.obj_name = f"s3-phtf-{self.deploy_env.value}-data-lake-athena-workgroup"
        super().__init__(
            scope,
            id=self.obj_name,
            name=self.obj_name,
